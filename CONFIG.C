 ...
 * configuration box.
 * CONTROLS ALL OF THE MAIN DIALOG CONTROLS
 */
...
//Mod by angelo to add smartcard
void conf_certsel_handler(union control *ctrl, void *dlg,
			  void *data, int event)
{
    int key = ctrl->fileselect.context.i;
    Conf *conf = (Conf *)data;
		      ...
		      int protocol, int protcfginfo)
{
	...
	if (!midsession) {
		...
		ctrl_certsel(s, "Certificate for authentication:", 'k',
			 FILTER_KEY_FILES, FALSE, "Select certificate file from store",
			 HELPCTX(ssh_auth_privkey),
			 conf_filesel_handler, I(CONF_keyfile));
		//TODO: Add ability to query for smartcard here
		//	also Dialog box changes can be made using win_res2.rc 
		//	in the directory above msvc
